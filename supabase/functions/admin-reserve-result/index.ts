...
Deno.serve(async (req: Request) => {
  ...
  const reservedResult =
    body?.reserved_result ?? body?.reservedResult ?? body?.result ?? null;
  ...
  if (!Number.isFinite(roundNumber) || roundNumber <= 0) {
    return json({ success: false, error: "Invalid round_number" }, 400);
  }

  const patch: Record<string, unknown> = {
    reserved_result:
      action === "cancel" || action === "remove" ? null : reservedResult,
    reserved_by:
      action === "cancel" || action === "remove" ? null : userData.user.id,
    reserved_at:
  ...
  const { data, error } = await supabase
    ...
    .not("status", "in", "(completed,settled)")
    .select("*")
    .maybeSingle();
