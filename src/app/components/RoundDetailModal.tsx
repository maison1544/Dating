...
export function RoundDetailModal({ round, onClose }: RoundDetailModalProps) {
  ...
  // 모의 배팅 유저 데이터 생성
  const mockBetUsers: BetUser[] = round.betDistribution?.flatMap((bet, idx) => 
    Array.from({ length: Math.max(1, Math.floor(bet.count / 2)) }, (_, i) => {
      const userId = idx * 100 + i;
      return {
        id: userId,
        userEmail: `user${String(userId).padStart(3, '0')}@example.com`,
        userNickname: ['김민수', '이서연', '박지훈', '최유진', '정하늘', '강서현', '윤지호', '송민아'][userId % 8],
        userName: ['김민수', '이서연', '박지훈', '최유진', '정하늘', '강서현', '윤지호', '송민아'][userId % 8],
        betType: bet.option,
        amount: Math.floor(bet.amount / Math.max(1, Math.floor(bet.count / 2))),
        result: round.status === "완료" 
          ? (bet.option.includes(round.result) ? "승리" : "패배")
          : "대기",
        timestamp: round.startTime,
        winAmount: Math.floor(bet.amount / Math.max(1, Math.floor(bet.count / 2))) * (bet.option.includes(round.result) ? 1.5 : 0),
        userIp: `192.168.${(userId * 7 + 13) % 255}.${(userId * 11 + 37) % 255}`,
      };
    })
  ) || [];

  // 배팅 옵션별 클릭 시 해당 유저 목록 표시
  const selectedOptionUsers = selectedBetOption
    ? mockBetUsers.filter(user => user.betType === selectedBetOption.option)
    : [];
          ...
          <div className="sticky top-0 bg-gray-900 border-b border-gray-800 p-6">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <span className={`${round.gameType === '파워볼' ? 'bg-purple-500/30 text-purple-300' : 'bg-blue-500/30 text-blue-300'} px-3 py-1 rounded text-sm font-semibold`}>
                  {round.gameType}
                </span>
            ...
            <div className="flex items-center gap-3 text-sm mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`font-semibold ${
                  (() => {
                    // 당첨금 계산 (승리한 배팅 옵션들의 금액 * 1.95 배당률)
                    const winAmount = round.status === "완료" && round.betDistribution
                      ? round.betDistribution
                          .filter(bet => bet.option.includes(round.result))
                          .reduce((sum, bet) => sum + bet.amount * 1.95, 0)
                      : 0;
                    const siteProfitLoss = round.totalAmount - winAmount;
                    return siteProfitLoss >= 0 ? "text-green-400" : "text-red-400";
                  })()
                }`}>
                  {(() => {
                    const winAmount = round.status === "완료" && round.betDistribution
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`font-semibold ${
                  round.status === "완료" ? "text-green-400" :
                  round.status === "진행중" ? "text-blue-400" :
                  "text-gray-400"
                }`}>
                  {round.status}
                </span>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex justify-center">
              <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 max-w-2xl w-full">
                <h3 className="text-white font-semibold mb-3 text-sm">배팅 분포도</h3>
                {round.betDistribution && (
                          ...
                          {round.gameType === '사다리' && combinationBets.length === 4 && (
                            <div className="space-y-2">
                              ...
                              <div className="flex w-full h-8 bg-gray-800 rounded-lg overflow-hidden">
                                {(() => {
                                  const hasNoCombinationBets = combinationBets.every(bet => bet.percentage === 0);
                                  return (
                                    <>
                                      <div
                                        onClick={() => setSelectedBetOption({
                                        ...
                                        style={{ width: hasNoCombinationBets ? '25%' : `${combinationBets[0].percentage}%` }}
                                      >
                                        {!hasNoCombinationBets && combinationBets[0].percentage > 0 && (
                                      ...
                                      <div
                                        ...
                                        style={{ width: hasNoCombinationBets ? '25%' : `${combinationBets[1].percentage}%` }}
                                      >
                                        {!hasNoCombinationBets && combinationBets[1].percentage > 0 && (
                                      ...
                                      <div
                                        ...
                                        style={{ width: hasNoCombinationBets ? '25%' : `${combinationBets[2].percentage}%` }}
                                      >
                                        {!hasNoCombinationBets && combinationBets[2].percentage > 0 && (
                                      ...
                                      <div
                                        ...
                                        style={{ width: hasNoCombinationBets ? '25%' : `${combinationBets[3].percentage}%` }}
                                      >
                                        {!hasNoCombinationBets && combinationBets[3].percentage > 0 && (
                ...
                <div className="overflow-x-auto max-h-96">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-700">
                      {(selectedBetOption ? selectedOptionUsers : mockBetUsers).map((user) => (
                        <tr key={user.id} className="hover:bg-gray-700/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-1 rounded text-xs ${
                              user.result === "승리" ? "bg-green-500/20 text-green-300" :
                              user.result === "패배" ? "bg-red-500/20 text-red-300" :
                          ...
                          <td className="px-4 py-3 text-right text-sm">
                            <span className={user.winAmount > 0 ? "text-yellow-400" : "text-gray-400"}>
                              {user.winAmount > 0 ? `+${user.winAmount.toLocaleString()}P` : "-"}
                            </span>
                          </td>
